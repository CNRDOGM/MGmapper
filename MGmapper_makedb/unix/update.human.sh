...
wget -nH --cut-dirs=3 -P $downloadLOG ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vertebrate_mammalian/Homo_sapiens/assembly_summary.txt -o $downloadLOG/assembly_summary.log
cat $downloadLOG/assembly_summary.txt | perl -ane '{@w=split(/\t/); if ($w[4] eq "reference genome") {print "$_"}}' > $subset
...
remark=`cat $subset | cut -f16`
echo "Date: $DATE" > ${metagenomicsDB}/${organism}.info
echo "Remark: Human reference sequence $remark" >> ${metagenomicsDB}/${organism}.info
...
taxFile=${metagenomicsDB}/${organism}.tax
${perl_scripts}/tax.pl -p $taxonomyDir -i $downloadLOG/seqName_taxid.txt -t taxid -o $taxFile -v -l ${metagenomicsLOG}/tax.log

#
# require Kyoto Cabinet to be installed under perl (http://fallabs.com/kyotocabinet/)
#
${perl_scripts}/insertTaxonomy.pl -i $taxFile -k ${metagenomicsDB}/${organism}.kch -v -l ${metagenomicsLOG}/insertTaxonomy.log -s
...
#${scriptPath}/makeblastdb.sh ${metagenomicsDB}/$organism ${metagenomicsDB} $organism >& /dev/null
