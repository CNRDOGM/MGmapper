...
if [ ! -d "$mirrorPath/$organism/refseq" ]; then
  mkdir -p $mirrorPath/$organism/refseq
fi
...
wget -nH --cut-dirs=3 -P $downloadLOG ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/viral/assembly_summary.txt -o $downloadLOG/assembly_summary.log
assembly=${downloadLOG}/assembly_summary.txt
cat  $assembly | perl -ane '{@w=split(/\t/); if ( (($w[10] eq "latest") && ($w[13] eq "Full")) && ( ($w[11] eq "Complete Genome") || ($w[11] eq "Chromosome") || ($w[4] eq "representative genome") || ($w[4] eq "reference genome")) ) {print "$_"}}' > $subset


echo "Date: $DATE" > ${metagenomicsDB}/${organism}.info
echo "Remark: genome_rep=Full; assembly_lvl=Complete Genome,Chr or refseq_cat=ref genome,rep genome" >> ${metagenomicsDB}/${organism}.info


${perl_scripts}/parse_assembly_summary.pl -i $subset -o $downloadLOG/refseq_wget.sh -w $downloadLOG/wget.log -m $mirrorPath/$organism/refseq -l ${downloadLOG}/parse_assembly_summary.log -v -t $downloadLOG/taxid_filename.txt


chmod a+rx $downloadLOG/refseq_wget.sh
/bin/tcsh $downloadLOG/refseq_wget.sh
...
#
# require Kyoto Cabinet to be installed under perl (http://fallabs.com/kyotocabinet/)
#
